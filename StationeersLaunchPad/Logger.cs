...
namespace StationeersLaunchPad
{
  ...
  public class Logger
  {
    public static Logger Global { get; } = new Logger("Global");
    ...
    public Logger(string name = "", Logger parent = null, int size = LogBuffer.DEFAULT_BUFFER_SIZE)
    {
      this.Name = name;
      this.Parent = parent;
      this.Buffer = new LogBuffer(this.IsChild ? 256 : size);
    }
    ...
    public LogLine At(int index) => index >= 0 && index < this.Count ? this.Buffer[index] : null;
